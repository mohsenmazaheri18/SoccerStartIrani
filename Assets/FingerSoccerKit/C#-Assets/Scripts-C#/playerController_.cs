...
public class playerController_ : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (Game_Manager.TurnedPlayer1.PlayersTurn && gameObject.CompareTag("Player_2"))
			selectionCircle.GetComponent<Renderer>().enabled = true;
		else if (Game_Manager.TurnedPlayer1.OpponentsTurn && gameObject.CompareTag("Player"))
			selectionCircle.GetComponent<Renderer>().enabled = true;
		else
	...
	void OnMouseDrag()
	{
		if (canShoot && ((Game_Manager.TurnedPlayer1.PlayersTurn && gameObject.CompareTag("Player")) ||
		                 (Game_Manager.TurnedPlayer1.OpponentsTurn && gameObject.CompareTag("Player_2"))))
		{
			//print("Draged");
			currentDistance = Vector3.Distance(helperBegin.transform.position, transform.position);

			//limiters
			if (currentDistance <= Game_Manager.maxDistance)
				safeDistance = currentDistance;
			else
				safeDistance = Game_Manager.maxDistance;
			...
			var position2 = helperBegin.transform.position;
			Vector3 dxy = position2 - position;
			float diff = dxy.magnitude;
			var position1 = position + ((dxy / diff) * currentDistance * -1);
			...
			Debug.DrawLine(position3, arrowPlane.transform.position, Color.blue);
			//debug line from initial position to the exact opposite position (mirrored) of our current touch position
			Debug.DrawLine(position3, (2 * position3) - position2, Color.yellow);
			...
			shootDirectionVector = Vector3.Normalize(position2 - position3);
	...
	void CastRay()
	{
		...
		_ray = new Ray(position1, (helperEnd.transform.position - position1).normalized);

		if (Physics.Raycast(_ray, out _hitInfo, currentDistance))
		{
			GameObject objectHit = _hitInfo.transform.gameObject;

			//debug line whenever the ray hits something.
			Debug.DrawLine(_ray.origin, _hitInfo.point, Color.cyan);

			//draw reflected vector like a billiard game. this is the out vector which reflects from targets geometry.
			Vector3 reflectedVector = Vector3.Reflect((_hitInfo.point - _ray.origin), _hitInfo.normal);
			Debug.DrawRay(_hitInfo.point, reflectedVector, Color.gray, 0.2f);

			//draw inverted reflected vector (useful for fine-tuning the final shoot)
			var position = _hitInfo.transform.position;
			Debug.DrawRay(position, reflectedVector * -1, Color.white, 0.2f);

			//draw the inverted normal which is more likely to be similar to real world response.
			Debug.DrawRay(position, _hitInfo.normal * -3, Color.red, 0.2f);
	...
	void ManageArrowTransform()
	{
		...
		else
		{
			...
			/*arrowPlane.transform.position = transform.position + ((dxy / diff) * Game_Manager.maxDistance * -1);
		...
		//for two players game, we need to set the aim attribute of each side separately. 
	...
	void OnMouseUp()
	{
		...
		if ((Game_Manager.TurnedPlayer1.PlayersTurn && gameObject.CompareTag("Player_2")) ||
		    (Game_Manager.TurnedPlayer1.OpponentsTurn && gameObject.CompareTag("Player")))
		{
			arrowPlane.GetComponent<Renderer>().enabled = false;
			shootCircle.GetComponent<Renderer>().enabled = false;
			aimArrowBody.GetComponent<Renderer>().enabled = false;
		...
		if (Game_Manager.TurnedPlayer1.PlayersTurn && gameObject.CompareTag("Player"))
		{
			outPower *= (1 + (TeamsManager.getTeamSettings(PlayerPrefs.GetInt("PlayerFlag")).x / 35.0f));
			//print ("[P1] Team Power: " + TeamsManager.getTeamSettings(PlayerPrefs.GetInt("PlayerFlag")).x);
		...
		StartCoroutine(gameController.GetComponent<Game_Manager>().ManagePostShoot(gameObject.tag));
	}
